...
mkdir build
cd build
d:\cmake\bin\cmake -G Ninja -DCMAKE_TOOLCHAIN_FILE=D:/vcpkg/scripts/buildsystems/vcpkg.cmake -DCMAKE_C_COMPILER="D:/one/compiler/latest/bin/icx.exe" -DCMAKE_CXX_COMPILER="D:/one/compiler/latest/bin/icx.exe" -DSITE="win-10" -DBUILDNAME="dev/ninja/icx-2025.3.0" -DCMAKE_BUILD_TYPE=Debug -DCLIO_CORE_ENABLE_RUNTIME=ON -DCLIO_CORE_ENABLE_CTE=ON -DCLIO_CORE_ENABLE_CAE=ON -DCLIO_CORE_ENABLE_CEE=OFF -DCLIO_CORE_ENABLE_TESTS=ON -DCLIO_CORE_ENABLE_PYTHON=OFF -DCLIO_CORE_ENABLE_ZMQ=ON -DCLIO_CORE_ENABLE_CEREAL=ON -DCLIO_CORE_ENABLE_CONDA=OFF -DCLIO_CORE_ENABLE_ELF=OFF -DCLIO_CORE_ENABLE_RPATH=OFF -DCLIO_CTE_ENABLE_VFD=OFF -DCLIO_CTE_ENABLE_HDF5_VOL=OFF -DCLIO_CORE_ENABLE_BENCHMARKS=OFF -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCLIO_CTP_LOG_LEVEL=0 ..
